...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_maps2", 		Command_MapChoose, 					"Show map list to begin vote for changelevel / set mark (rating) etc.");
	//TODO
...
void Actualize_MapChangerInfo()
{
	...
	if( fWrite )
	{
		...
		KeyValuesToFileW(g_kvinfo, g_sMapInfoPath);
	}
...
bool ParseMissionFile(char[] missionFile, bool bIsAddon)
{
	...
	if( bIsAddon )
	{
		...
		KeyValuesToFileW(g_kv, g_sMapListPath);
	}
	return true;
}
...
stock bool KeyValuesToFileW(KeyValues kv, const char[] file) // thanks to @Kailo
{
